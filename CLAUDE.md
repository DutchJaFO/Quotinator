...
2. **Always use code-behind files** — every `.razor` file has a paired `.razor.cs` partial class, even if it contains only the namespace and class declaration. No inline `@code { }` blocks, no `@inject` directives. Move `@inject` to `[Inject]` properties and `@using System.*` to the `.razor.cs` using list. The only exception is if the Blazor framework itself does not support a code-behind partial for that file type.
