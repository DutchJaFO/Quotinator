...
**When adding a persisted column backed by a C# enum — checklist (same migration), per [ADR 008](docs/architecture-decisions/008-enum-backed-columns-require-check-constraints.md):**
1. Add a `CHECK (col IN (...))` clause enumerating the enum's current members.
2. If the column is added via `ALTER TABLE ... ADD COLUMN`, that statement can carry the `CHECK` inline — SQLite supports it (confirmed against sqlite.org). Only widening or otherwise changing an *existing* CHECK'd column's constraint needs a table-rebuild migration (create-new-table + copy + drop + rename), since SQLite has no `ALTER TABLE ... MODIFY CHECK`.
3. Update the baseline SQL to match.
4. Update the schema-drift CHECK-constraint test.
