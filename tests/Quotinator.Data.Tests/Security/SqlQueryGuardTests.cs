...
{
    /// <summary>
    /// Every non-id text column Data's own entities carry — discovered via reflection, not a
    ...
    /// <summary>
    /// Reflects over every string constant in <see cref="Sql"/> and its nested classes, and asserts
    /// none compare a non-id text column (Status/EntityType/TableName discriminator, etc.) to a bound
