...
public class SqliteImportActionServiceTests
{
    ...
    public async Task ApplyBatchAsync_MarkCompletenessAsProvided_OverridesAutoCompute()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var completenessStatus = await conn.ExecuteScalarAsync<string>("SELECT CompletenessStatus FROM Quotinator_Quote WHERE Id = @id", new { id });
    ...
    public async Task ApplyBatchAsync_QuoteAlreadyComplete_MarkCompletenessAsOmitted_StatusUnchanged()
    {
        ...
        {
            preDecideConn.Open();
            var textBeforeDecide = await preDecideConn.ExecuteScalarAsync<string>("SELECT QuoteText FROM Quotinator_Quote WHERE Id = @id", new { id });
        ...
        verifyConn.Open();
        var completenessStatus = await verifyConn.ExecuteScalarAsync<string>("SELECT CompletenessStatus FROM Quotinator_Quote WHERE Id = @id", new { id });
        ...
        var textAfterApply = await verifyConn.ExecuteScalarAsync<string>("SELECT QuoteText FROM Quotinator_Quote WHERE Id = @id", new { id });
    ...
    public async Task ApplyBatchAsync_BrandNewQuote_WritesQuoteAndSourceRows()
    {
        ...
        var result = await _service.ApplyBatchAsync(batchId.ToString("D").ToUpperInvariant(), cancellationToken: TestContext.CancellationToken);
    ...
    public async Task GetPagedAsync_And_ApplyBatchAsync_LowercaseBatchId_StillMatchesUppercaseStoredValue()
    {
        ...
        var page = await _service.GetPagedAsync(lowercaseBatchId, null, null, 1, 50, TestContext.CancellationToken);
        Assert.AreEqual(2, page.TotalCount, "Quote + Source actions for a brand-new quote with no character");

        var result = await _service.ApplyBatchAsync(lowercaseBatchId, cancellationToken: TestContext.CancellationToken);
    ...
    public async Task ApplyResolvedActionAsync_ReAddAfterSoftDelete_ResurrectsSoftDeletedRow()
    {
        ...
        {
            conn.Open();
            var sourceId    = await conn.ExecuteScalarAsync<Guid>("SELECT SourceId FROM Quotinator_Quote WHERE Id = @id", new { id = quoteId });
            var characterId = await conn.ExecuteScalarAsync<Guid>("SELECT CharacterId FROM Quotinator_Quote WHERE Id = @id", new { id = quoteId });
            var now = DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss");
        ...
        var result = await _service.ApplyBatchAsync(batch2.ToString("D").ToUpperInvariant(), cancellationToken: TestContext.CancellationToken);
    ...
    public async Task GetPagedAsync_BrandNewQuote_QuoteActionRelatesToItsSourceAndCharacterActions()
    {
        ...
        var page = await _service.GetPagedAsync(batchId.ToString("D").ToUpperInvariant(), null, null, 1, 50, TestContext.CancellationToken);
    ...
    public async Task GetPagedAsync_FilterByEntityType_ReturnsOnlyMatchingRows()
    {
        ...
        var page = await _service.GetPagedAsync(batchId.ToString("D").ToUpperInvariant(), null, "Source", 1, 50, TestContext.CancellationToken);
    ...
    public async Task GetPagedAsync_FilterByStatusLowercase_StillMatchesStoredValue()
    {
        ...
        var page = await _service.GetPagedAsync(batchId.ToString("D").ToUpperInvariant(), "decided", null, 1, 50, TestContext.CancellationToken);
    ...
    private async Task<Guid> StageAndApplyConversationAsync(string quoteId, string stageDirectionId, string conversationId)
    {
        var batchId = Guid.NewGuid();
        var quote = BuildQuote(quoteId);
        var stageDirection = new SourceStageDirectionDto { Id = stageDirectionId, Text = "[A stage direction]" };
        var conversation = new SourceConversationDto
        {
            Id = conversationId,
            Lines =
            [
    ...
    public async Task ReverseBatchAsync_TwoPhaseFlowBatch_SucceedsWithoutManualStatusOverride()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var textAfterReverse = await conn.ExecuteScalarAsync<string>("SELECT QuoteText FROM Quotinator_Quote WHERE Id = @id", new { id });
    ...
    public async Task ReverseBatchAsync_StageDirectionStillReferencedByAnotherConversation_IsKeptNotSoftDeleted()
    {
        ...
        var conversation2 = new SourceConversationDto
        {
            Id = conversation2Id,
            Lines =
            [
    ...
    public async Task ReverseBatchAsync_QuoteModify_RestoresExistingFields()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var text = await conn.ExecuteScalarAsync<string>("SELECT QuoteText FROM Quotinator_Quote WHERE Id = @id", new { id });
    ...
    public async Task ReverseBatchAsync_QuoteModify_SourceTextChanged_RestoresOriginalLinkage()
    {
        ...
        {
            conn.Open();
            var changedSourceId = await conn.ExecuteScalarAsync<Guid>("SELECT SourceId FROM Quotinator_Quote WHERE Id = @id", new { id });
        ...
        using var verifyConn = new SqliteConnection($"Data Source={_dbPath}");
        verifyConn.Open();
        var restoredSourceId = await verifyConn.ExecuteScalarAsync<Guid>("SELECT SourceId FROM Quotinator_Quote WHERE Id = @id", new { id });
    ...
    public async Task ReverseBatchAsync_QuoteModify_RestoresExistingBatchId()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var restoredBatchId = await conn.ExecuteScalarAsync<Guid>("SELECT ImportBatchId FROM Quotinator_Quote WHERE Id = @id", new { id });
    ...
    public async Task ReverseBatchAsync_SkipPolicyModify_NoWriteButReversesCleanly()
    {
        ...
        using var verifyConn = new SqliteConnection($"Data Source={_dbPath}");
        verifyConn.Open();
        var textAfter = await verifyConn.ExecuteScalarAsync<string>("SELECT QuoteText FROM Quotinator_Quote WHERE Id = @id AND IsDeleted = 0", new { id });
    ...
    public async Task ReverseBatchAsync_WritesSystemChangeLogEntries()
    {
        ...
        var sourceDeleted = await conn.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM Quotinator_Source WHERE IsDeleted = 1");
    ...
    public async Task ApplyBatchAsync_SourceModify_WritesResolvedFields()
    {
        ...
        var result = await _service.ApplyBatchAsync(sourceAction.BatchId, cancellationToken: TestContext.CancellationToken);
    ...
    public async Task ApplyBatchAsync_SourceModifyWithExplicitNullDate_ClearsDate()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var date = await conn.QuerySingleAsync<string?>("SELECT Date FROM Quotinator_Source WHERE Id = @id", new { id });
    ...
    public async Task ApplyBatchAsync_SourceSeriesIdDecided_WritesResolvedSeriesId()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var storedSeriesId = await conn.QuerySingleAsync<string?>("SELECT SeriesId FROM Quotinator_Source WHERE Id = @id", new { id });
    ...
    public async Task ApplyBatchAsync_LowercaseExplicitSourceId_SecondBatchCorrection_NeverCreatesDuplicateRow()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var count = await conn.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM Quotinator_Source WHERE Title LIKE 'T2 Regression Movie%'");
        ...
        // #209: the row is stored under its canonicalized (uppercase) id, not the lowercase id this
        // test declared — UPPER() makes the verification query tolerant of either.
        var title = await conn.ExecuteScalarAsync<string>("SELECT Title FROM Quotinator_Source WHERE UPPER(Id) = UPPER(@id)", new { id = sourceId });
    ...
    public async Task ApplyBatchAsync_LowercaseExplicitSourceId_MasterdataRepositoryLookupResolves()
    {
        ...
        var repository = new SqliteRestorableRepository<SourceEntity>(_factory, NoOpAuditEntryWriter.Instance, NoOpCallerContext.Instance);
    ...
    public async Task ReverseBatchAsync_ConversationModify_RestoresDescriptionOnly()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var description = await conn.ExecuteScalarAsync<string>("SELECT Description FROM Quotinator_Conversation WHERE Id = @id", new { id });
        Assert.AreEqual("A tense standoff.", description, "Reversal must restore the pre-Modify description");

        var lineCount = await conn.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM Quotinator_ConversationLine WHERE ConversationId = @id", new { id });
    ...
    public async Task ApplyBatchAsync_PersonAdd_WritesDateOfBirthAndDateOfDeath()
    {
        ...
        var result = await _service.ApplyBatchAsync(action.BatchId, cancellationToken: TestContext.CancellationToken);
        Assert.IsNull(result);
    ...
    public async Task ReverseBatchAsync_CharacterModify_RestoresExistingValue()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var name = await conn.ExecuteScalarAsync<string>("SELECT Name FROM Quotinator_Character WHERE Id = @id", new { id });
    ...
    public async Task ReverseBatchAsync_CharacterAdd_ExplicitLowercaseId_SoftDeletesCorrectly()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var isDeleted = await conn.ExecuteScalarAsync<int>("SELECT IsDeleted FROM Quotinator_Character WHERE UPPER(Id) = UPPER(@id)", new { id });
    ...
    public async Task DecideAsync_SeriesModify_ResolvesFieldDecisionsAndAppliesToDisk()
    {
        ...
        var stillPending = await _service.ApplyBatchAsync(action.BatchId, cancellationToken: TestContext.CancellationToken);
        Assert.IsNull(stillPending, "The batch must apply cleanly once its only action is decided");

        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var name = await conn.ExecuteScalarAsync<string>("SELECT Name FROM Quotinator_Series WHERE Id = @id", new { id });
    ...
    public async Task DecideAsync_UniverseModify_ResolvesFieldDecisionsAndAppliesToDisk()
    {
        ...
        using var conn = new SqliteConnection($"Data Source={_dbPath}");
        conn.Open();
        var name = await conn.ExecuteScalarAsync<string>("SELECT Name FROM Quotinator_Universe WHERE Id = @id", new { id });
    ...
    public async Task BulkDecideAsync_ValidRows_DecidesTheAction()
    {
        ...
        var response = await _service.BulkDecideAsync(batchId.ToCanonicalId(), rows, TestContext.CancellationToken);
    ...
    public async Task ExportThenBulkDecide_ViaCsvWireFormat_RoundTripsWithZeroErrors()
    {
        ...
        var response = await _service.BulkDecideAsync(batchId.ToCanonicalId(), parsedRows, TestContext.CancellationToken);
