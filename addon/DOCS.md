...
| `GET /api/v1/import/actions/export?batchId=&format=` | Export every decidable field across `batchId`'s Pending/Decided/Blocked Modify actions as a flat row list — `format=json` (default) or `format=csv`. A Decided action's `Decision`/`CustomValue` reflect the caller's actual prior choice, not an inference |
