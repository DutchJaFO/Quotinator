...
For the full, current endpoint list — every route, its parameters, and response shape — use the interactive Scalar reference at `/scalar/v1` (under whichever access path you use) or the raw OpenAPI spec at `/openapi/v1.json`. A plain-text version with the same content lives in the repository at [`docs/api-endpoints.md`](https://github.com/DutchJaFO/Quotinator/blob/main/docs/api-endpoints.md).
