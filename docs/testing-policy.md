...
its own terms (`QuotinatorWebApplicationFactory`, which waits for `StartupPhaseState.IsComplete`), and
held there by a guard in three independent layers that stops any test reaching the bare factory: a
**source analysis** that resolves every construction, base type, generic instantiation and `typeof` in
the project to its symbol, so no spelling can hide one; an **assembly analysis** that reads the compiled
IL, so it judges what the compiler emitted; and a **runtime guard** that fails any `Program` host built
without the guarded factory, for construction neither static layer can see. Both static layers are held
to the same verdict over a set of code samples, each shown finding what it should before its "found
nothing" over the real project counts. Treat an
...
containing at least one alphabetic hex character (`a` to `f`, either case). An all-digit literal (e.g.
...
`"aabbccdd-1234-4abc-8def-1234567890ab"` (contains `a` to `f`). Applies to every `*Id`-suffixed column
...
1. Add the key to `i18ntext/UI.en-GB.json` (the baseline and source of truth)
