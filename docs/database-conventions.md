...
| ✅ Do | Trust the automated guard, not memory, as the backstop — `Quotinator.Data.Diagnostics.SqlIdCaseGuard` (#210) scans every `Sql.cs` constant/factory method and every `RepositorySql.cs` factory method (both fields *and* arrow-bodied properties) for any id-column comparison, including a JOIN, that isn't `UPPER()`-wrapped on both sides, and fails the build if one is found. A half-wrapped comparison (only one side) is flagged too. `IdClauses` is what you write with; the guard is what catches the case where it wasn't used. |
...
| ❌ Don't | Assume a comparison is safe because its only known caller already supplies matching casing — the guard exists precisely because a *future* caller is not bound by today's callers' behaviour. Wrap every id comparison in `UPPER(...)` regardless of whether it looks safe today. |
