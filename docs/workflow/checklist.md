...
- [ ] **Boyscout pass over the files this issue touched** — always part of closing an issue, never optional and never a judgement call about whether it is "bigger than this issue needs". Scope is exactly the files this issue created or touched: a breach in an *untouched* file is surfaced to the developer instead, per `CLAUDE.md` → "Zero-warnings policy and boyscout rules", which is also where each rule below is stated in full.
  - [ ] Every touched file added to `.editorconfig`'s scoped `IDE0008` and `IDE0090` sections, with both conversions run until the warning counts stop dropping, and the follow-on `IDE0028`/`IDE0305` warnings they expose fixed in the same pass
  - [ ] `[Subsystem - Phase]` prefix added to any log line in a touched file that lacks it (`docs/logging.md`)
  - [ ] `nameof(TEntity.Property)` used for column names in any `Sql.*` nested class touched
  - [ ] Any code-style rule a touched file breaches **with no reason stated at the breach** is brought into line — enum placement (ADR 016), file/namespace placement and Blazor code-behind (`CLAUDE.md`). A stated reason is a decision; silence is a deviation, and a deviation gets fixed
