...
| 11 | ✅ | T2 verified | Live | `docs/smoke-tests.md` §31 (now `docs/automated-testing/`, whose README maps the old section numbers) — Docker build + live container: date-range/export endpoints, `Content-Disposition` header, row-count cap (422), auto-purge on (0 remaining `Import_Action` rows, 4 `Purged` audit traces) vs. off (1365 rows retained), `purgeOnSuccess=true` on a live import followed by `POST /import/actions/reverse` returning 422 |
