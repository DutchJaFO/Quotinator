# #308 â€” Notification: multi-line/rich message layout
...
`NotificationTable` renders its message as a single plain cell with no line-break handling â€” Razor
...
notification type* is laid out across *both* surfaces â€” the startup/popup dialogs and the
`/notifications` view â€” and each remaining producer brings a type with its own payload. Designing those
layouts before the types exist is guessing.

## Scope revision â€” no longer rendering-only
...
Both were revised. The milestone's goal was restated â€” v1.8.0 shipped a *basic* notification system
and this milestone makes it complete â€” which made #278's flat `Message` the bottleneck rather than a
...
- **The issue's own *Expected tests* table hedged**: "a rendering/formatting-level test if one is
  feasible without bUnit â€” otherwise a Live/T1 verification step". That hedge is false. This project
  has a settled pattern for exactly this â€” `AwaitingReview`, `DecisionRows`, `FileNameFor`,
...
## Reopened, 2026-09-02 â€” four findings from T1
...
1. **A resolved notification does not say how it was resolved.** The Status column reads *Done* while
   the body still reads "1 changes need your decision before they can be applied" â€” which stopped being
   ...
   stored nowhere** â€” `DecideBatchAsync` receives the `FieldResolutionChoice` and discards it.
...
3. **The bodies are too long for a flat table row.** Presentation: **dialog on the `/notifications`
   page, collapsible in the startup popup** â€” the page is a long scannable history where rows growing
   ...
   Detail renders as a **table**, not a list â€” every entry has the same shape, so columns line the
...
4. **`Run` does not say what it will do**, and for a multi-outcome action the choices are hidden behind
   it. The available actions should be named on their own buttons â€” **reusing the names the actions
   ...
   the body in that very row read "Run a reseed to load the bundled sources" â€” a synonym for the
   ...
   `.WithSummary("Reset the database")` on `AdminEndpoints`, and `ImportReviewDecideColumn` â€”
   *"Decide"* â€” is already the review page's own heading for these very controls.
   ...
   button label, look for the name the operation already has** â€” an endpoint summary, a column heading,
   an existing UI key.
...
had to be found and corrected separately â€” precisely the drift CLAUDE.md's duplication rule describes.
...
per-type layout across both surfaces". What it produced was `LayoutFor(kind) â†’ BodyIsMultiLine`, a
line-wrapping boolean â€” the minimum that satisfied its own test. A per-type layout is what decides
...
database. The reversal is deliberate and scoped â€” one nullable, enum-backed column with its CHECK
...
## Reopened, 2026-09-22 â€” the per-type layout was never implemented

Step 4 says each type gets a layout decision across both surfaces. What exists is
`NotificationTable.LayoutFor(kind) â†’ NotificationLayout(BodyIsMultiLine, PayloadParts)`, and **no
...
| Commit | What it did |
|---|---|
| `69c01776` | `LayoutFor` born as `BodyIsMultiLine` alone â€” a line-wrapping boolean |
| `040a14d6` | `PayloadParts` added to the record with every arm empty, plus the tests that assert it |
| `a799e91a` | The rendering built as `PayloadLines(notification, text)`, switching on the payload type â€” never reading the map |
...
a test of the feature, so there was never a red the feature could turn green â€” which is why row 23 is
...
evidence instead of being decided up front â€” if the per-variant assertions can be written without it, it
...
a written list, so defining a new content variant fails these tests until it is handled** â€” the property
...
every test â€” unit *and* automated â€” and runs them before any implementation exists. Steps 2â€“5 turn
named rows green. No implementation step precedes its own test.

### 1. Write every test first, and run them red

**Status:** âœ… Done â€” six unit tests red, T2 document written and run red
...
- **For code that does not exist yet, add the signature with a deliberately wrong body** â€” the
...
- **Write the T2 document now, and run it now.** At this moment `HEAD` *is* the pre-work build, so the
  canary `docs/testing-policy.md` requires costs nothing â€” no worktree, no second image. Running the
...
**Status:** âœ… Done â€” turns rows 1â€“3 green

Its own heading or emphasis, not one undifferentiated cell. A notification with no title still renders
correctly â€” `Title` is nullable in #312's schema, and the two shipped producers (#279, #289) predate
...
**Status:** âœ… Done â€” turns rows 4â€“5 green
...
rule applies.** That is the #303 trap exactly â€” the nav icon's class was present while the icon was
...
**Status:** âœ… Done â€” turns rows 6â€“7 green

Each notification type gets a layout decision for the startup/popup dialog and for the
`/notifications` view. Six kinds exist as of 2026-09-01 â€” `Announcement`, `SchemaVersionOvershoot`,
`WhatsNew`, `ReseedRecommended`, `ReseedFileApplied`, `ImportReviewPending` â€” plus rows with no
...
**Status:** âœ… Done â€” turns row 8 green. **Retitled 2026-09-02**; see below.
...
**Status:** âœ… Done â€” turns rows 9â€“11 green

`NotificationSummary` (the startup modal) and the `/notifications` page both consume
`NotificationTable`. A change that looks right in one can be wrong in the other â€” the modal is
size-constrained in a way the page is not.

---

### 7. Write the tests for findings 1â€“4, and run them red

**Status:** âœ… Done â€” eight tests red across two opposing-stub runs
...
run is not a fresh canary â€” it is the same proof applied to the rows being added, at the only moment it
is free.
...
**Row 23 was mis-specified twice, and both were planning failures â€” not findings** (developer,
2026-09-02).

**First**, it asserted that payload rendering never replaces the one-line body â€” true, but expressed as
...
choice assumed some type might show detail *instead of* its summary. **The body is always relevant â€”
...
expressible is not a requirement â€” and a correction made without re-checking the premise is how one
wrong row becomes two.

**A storage-backed test cannot reach its assertion before its column exists** â€” recorded 2026-09-02
...
Resolution"*, not on an `Assert`. That is a genuine red â€” the behaviour is absent â€” but it is weaker
...
**Status:** âœ… Done â€” rows 17â€“20 green
...
the property without its column broke **76 tests** â€” every notification write in the solution, not the
...
**The rendered line is a translated key, never a stored sentence** â€” #319 owns the translation shape,
...
migration per ADR 008, the baseline updated to match in the same commit, and both drift tests extended â€”
...
member. **The rendered line is a translated key, never a stored sentence** â€” #319 owns the translation
...
**Status:** âœ… Done â€” rows 21â€“23 green

`LayoutFor` grows from a boolean into a per-type description of *which parts of the payload that type
shows* â€” the per-entity breakdown for `ReseedFileApplied`, the per-status counts and batch for
...
**Status:** âœ… Done â€” rows 24â€“26 green
...
circuit â€” an expander needing it adds nothing. The degraded case argues the other way too: on a
...
**Status:** âœ… Done â€” rows 27â€“29 green

A per-trigger label â€” reseed, reset â€” and for a multi-outcome action, both choices offered directly
...
**Status:** âœ… Done â€” rows 30â€“32 green
...
`ApplyBatchAsync` had already dismissed the row. Rows 17â€“18 missed all three because both read with raw
...
plan recorded them in prose and moved on, so nothing re-ran to catch a regression â€” against
`docs/testing-policy.md`'s rule that a bug fix ships with the test that would have caught it. Rows
34â€“36 close that, each proven red by reproducing the original defect: the resolution argument dropped,
...
recorded only the trigger â€” so *every* assertion about a dismissal passed while the caller sent
...
**Status:** âœ… Done â€” rows 33â€“34 green
...
`StartupErrorModal` and the detail popup pass content into it. **The cap is not a parameter** â€” a
...
sequence of its own because it was not planned â€” it was the third copy being noticed. Step 4's
...
per kind, constructed with the container stopped â€” the technique
[`01-notification-system.md`](../../automated-testing/notifications-and-changelog/01-notification-system.md)
step 7 already uses for rows no producer creates â€” each with a payload valid for its kind, then read on
...
- a detail control appears for exactly the kinds whose payload holds something their body does not, and
  for no others â€” a dialog on the page, an expander in the popup;
...
rendered under assertion at all** â€” both are covered through the API only, by
...
The unit tests derive their cases from `NotificationMetadataKind`, as row 6 already does. A live
document cannot enumerate a C# enum, so a guard test reads the document and asserts it names every
member â€” the shape `RepositoryStructureTests` already uses to hold documents to a rule. Defining a new
kind then fails the build until the document covers it.
...
- **If any assertion needs an expectation the payload type cannot supply**, the map is that expectation:
  `PayloadDetail` reads it, and a unit test asserts the rendered output matches what the map declares â€”
...
|---|--------|-------------|--------|--------------|
| 1 | âœ… | A notification with a title renders a title element | Unit test | `NotificationTableTests.ShowsTitle_WithATitle_IsTrue` |
| 2 | âœ… | A notification with no title renders none | Unit test | `NotificationTableTests.ShowsTitle_WithoutATitle_IsFalse` â€” `null`, `""` and whitespace, the shape #279/#289 rows have |
| 3 | âœ… | The title element is not rendered *instead of* the body | Unit test | `NotificationTableTests.ShowsTitle_WithoutATitle_StillRendersTheBody` â€” positive control, so row 2 cannot pass against a cell that renders nothing at all |
| 4 | âœ… | Markup and stylesheet agree on the body cell's class | Unit test | `NotificationTableTests.BodyCellClass_IsDefinedInTheStylesheet` â€” parses `NotificationTable.razor` and `.razor.css`; proves the two halves match, never that the rule applies |
| 5 | âœ… | A two-line body renders as two lines | Automated (T2) | new `13-notification-layout.md` â€” asserts computed `white-space: pre-line` **and** `getClientRects().length >= 2`, not the class name |
| 6 | âœ… | Every `NotificationMetadataKind` has a defined layout | Unit test | `NotificationTableTests.EveryMetadataKind_HasALayout` â€” derived from the enum, so a kind added later fails here |
| 7 | âœ… | A row with no metadata kind still has a layout | Unit test | `NotificationTableTests.NoMetadataKind_FallsBackToADefinedLayout` â€” negative case for row 6; #279/#289 rows carry none |
| 8 | âœ… | `System_Notification`'s column set changes only by decision | Unit test | `DatabaseInitializerOwnershipTests.SystemNotification_ColumnSet_IsPinned` â€” replaces "unchanged in the diff", which nothing re-runs. **Requirement revised 2026-09-02**: it read "adds no column", which the step 5 reversal overturned. The list now includes `Resolution` and was updated in the migration's own commit, which is what makes the addition deliberate |
| 9 | âœ… | Title and body render distinctly on `/notifications` | Automated (T2) + screenshot | `13-notification-layout.md` â€” the title is its own element in the DOM, not a prefix inside the body text |
| 10 | âœ… | The same holds in the startup modal | Automated (T2) + screenshot | same document â€” restart required, per #302's finding that the modal shows once per process run |
| 11 | âœ… | Rendering survives a degraded startup | Automated (T2) | same document â€” degraded container, parity with `/notifications`'s current behaviour rather than a bare `200` (see #303 row 36) |
| 12 | âœ… | The T2 document goes red before it goes green | Canary run | run at step 1 against `52071f24`, no worktree needed: `bodyCells: 0`, `titleElements: 0`, `whiteSpace: normal`, `lineBoxes: 1` â€” and step 1 itself failed on a wrong fixture assumption, corrected before implementing |
| 13 | âœ… | Every unit test above is wired to behaviour | Mutation | proven at step 1 by two opposing stubs: `ShowsTitle => false` fails rows 1, 4, 6, 7; `ShowsTitle => true` fails rows 2 and 3, which assert an absence and cannot fail against the first. Row 8 went red on a wrong column list before going green |
| 14 | âœ… | Build is clean | Build | `dotnet build --configuration Release` â†’ 0 warnings, 0 errors |
| 15 | âœ… | No regression | Test run | `dotnet test --configuration Release -m:1` all green |
| 16 | âŒ | Every layout renders correctly on the developer's own machine | Live (T1) | one confirmed rendering per type, on both surfaces |
| 17 | âœ… | A notification resolved by an action records which resolution it was | Unit test | `NotificationWriterTests.DismissedAsResolved_RecordsTheResolution` â€” proven by mutation (hard-coding a resolution fails it) |
| 18 | âœ… | A notification dismissed by the user records no resolution | Unit test | `NotificationWriterTests.DismissedByUser_RecordsNoResolution` â€” negative; the field means "how the action settled it", not "how it went inactive". Wired via `Sql.Notifications.UpdateDismissById`: the by-batch mutation does **not** reach this path, so proving it needed the by-id query mutated instead |
| 19 | âœ… | The migration and the baseline accept the same `Resolution` values | Unit test | `DatabaseInitializerOwnershipTests.DataOwnedBaseline_And_IncrementalReplay_AcceptSameNotificationCheckConstraintValues` â€” extended with all four members and a rejected value, on both paths |
| 20 | âœ… | Every `NotificationResolution` member has a label in all three locales | Unit test | `NotificationTableTests.EveryResolution_HasATranslationKey` â€” derived from the enum, like `EveryDisplayStatus_HasATranslationKey`; `TranslationCompletenessTests` covers the other two locales |
| 21 | â¬œ | A type that renders detail also names its columns | Unit test | `NotificationTableTests.PayloadDetail_ForEveryKind_IsSelfDescribing`. **Un-ticked 2026-09-02, found during #373: it could not fail.** Two independent causes, either sufficient on its own â€” the `WithTitle` helper never set `MetadataKind`, which `PayloadDetail` dispatches on; and `MetadataFor` never emitted `releaseState`, which `NotificationMetadataDto` declares `required`, so deserialization threw and `TryDeserialize` swallowed it. Every fixture yielded an empty table, and the assertion `AreEqual(rows > 0, headers > 0)` held as `false == false` for every kind. The rendering was correct throughout; the test measured nothing. #373 fixes both fixtures and adds enum-derived positive and negative coverage per kind â€” headers exist exactly when rows do, and every row's cell count matches. Replaced `ContentLines_ForEveryKind_LeadWithTheBody` when detail became a table: the body is now unconditional markup, so its precedence is proven by the T2 document, not by a unit assertion that cannot fail |
| 22 | âœ… | A payload that cannot be deserialised renders the plain body rather than throwing | Unit test | `NotificationTableTests.UnreadablePayload_FallsBackToTheBody` â€” negative; a row written by an older build must still render. Proven by mutation: making the fall-through arm return a line fails it |
| 23 | â¬œ | Whether a type has structured detail beneath the summary varies | Unit test | **Un-ticked 2026-09-22.** `NotificationTableTests.LayoutFor_AcrossKinds_PayloadDetailVaries` asserts the contents of `LayoutFor`'s own map, which no renderer reads â€” red while every arm was empty, green once two arms were filled in, neither state requiring the feature. Row 38 replaces it with what a reader sees |
| 24 | âœ… | The page opens payload detail in a dialog | Automated (T2) + screenshot | `13-notification-layout.md` step 7 â€” `expandersOnPage: 0`, `openButtonsOnPage: 4`; the dialog carries the table. **Swapped 2026-09-02**: the page originally collapsed and the popup opened a dialog |
| 25 | âœ… | The startup popup expands detail in place | Automated (T2) | same document step 8 â€” `expandersInModal: 4`, `openButtonsInModal: 0`, `collapsedByDefault: true` |
| 26 | âœ… | Detail renders as a table, never a list | Automated (T2) | same document step 7 â€” headings `Entity / Added / Updated` over `Quote \| 63 \| 0`; `listsAnywhere: 0` guards the regression |
| 27 | âœ… | Each executable trigger's button is named for what it does | Unit test | `NotificationTableTests.ActionLabelFor_EachExecutableTrigger_IsNamed` â€” derived from `NotificationDismissTrigger`, so a new one fails here. Confirmed live: the button reads *Reseed the database* â€” the endpoint summary verbatim. **Corrected twice on 2026-09-02**: *Reload the quotes*, then *Run a reseed*, both invented while the operation already had a name |
| 28 | âœ… | A multi-outcome action offers both choices without an intermediate click | Unit test | `NotificationTableTests.ImportReviewResolved_OffersBothChoicesDirectly` |
| 29 | âœ… | The confirmation step still stands | Automated (T2) | same document step 9 â€” clicking the named button yields `Confirm`/`Cancel`, and nothing runs until Confirm |
| 30 | â¬œ | Every layout renders on both surfaces | Automated (T2) + screenshot | **Un-ticked 2026-09-22.** The document's steps 7â€“10 read whichever kinds its fixture happened to produce â€” `ReseedRecommended`, `SchemaVersionOvershoot` and a row with no metadata kind have never been rendered under assertion on either surface. Rows 39â€“40 cover every kind, derived from the enum |
| 31 | âœ… | The new T2 steps go red before they go green | Canary run | `quotinator:canary308b` at `7bc5bacc`: `0` expanders, `0` open buttons, a button reading `Run`, `0` resolution lines. Step 9 needed an actionable row first â€” it passed vacuously without one |
| 32 | âœ… | A resolved notification says how it was resolved | Automated (T2) | same document step 10 â€” the resolved row reads *Database reseeded*, and `GET /notifications` returns `resolution: reseeded` |
| 33 | âœ… | No modal grows past the viewport, on any surface | Automated (T2) + screenshot | same document steps 7 and 8 â€” every modal renders through `ModalDialog`, capped at `95vh` with a scrolling body. Measured on a deliberately short `420px` viewport: detail popup `364`, startup popup `364`, both scrolling with header and footer reachable. **Found twice in T1 (developer, 2026-09-02)**: once on the startup popup, then again on the detail popup â€” which is what forced the shared control |
| 34 | âœ… | The reseed and reset paths record their resolution, not only their trigger | Unit test | `NotificationActionExecutorTests.ExecuteAsync_Reseed_...` and `..._DatabaseReset_...` â€” proven by mutation: dropping the resolution argument, as the shipped code originally did, fails both. **Added 2026-09-02** to close the gap step 12 exposed |
| 35 | âœ… | The stored resolution survives the read path | Unit test | `NotificationWriterTests.DismissedAsResolved_TheResolutionSurvivesTheReadPath` â€” reads through `NotificationReader`, not `SELECT *`. Proven by mutation: removing `n.Resolution` from the read query's column list fails it |
| 36 | âœ… | The by-trigger dismissal writes the resolution, not only the by-batch one | Unit test | `NotificationWriterTests.DismissedByTrigger_RecordsTheResolution` â€” the path reseed and reset actually take, which rows 17â€“18 did not exercise |
| 37 | âœ… | A fourth modal cannot reintroduce the duplication unnoticed | Automated (T2) | same document step 7 â€” `usesOldBespokeMarkup: false`, asserting the copied class names absent. **A regression guard, not a proven-red row**: the markup it forbids existed while the assertion did not, so unlike row 31 it has no canary run behind it, and the document says so |
...
| 41 | ⬜ | A kind defined later fails these tests until it is covered | Unit test | `NotificationTableTests` cases derived from `NotificationMetadataKind`, plus a guard test asserting the live document names every member |
...
icon is the standing example â€” the class was present the whole time the icon was missing.
