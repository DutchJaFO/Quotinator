...
| 33 | ✅ | Reseeding repeatedly leaves the pending count stable, not growing | Unit test | **Corrected a second time, 2026-09-07/08 — genuinely `0 / 0 / 0`, not a stable `3`.** The three tv-season conflicts accepted as permanent on 2026-09-04 turned out to be resolvable after all (developer directive: get reseed to zero via the existing `Custom`-rule mechanism, matching each show's own existing Source bucket; per-episode precision is separate future work). Covered by the consolidated `DatabaseInitializerTests.NikhilNamal17RealCorpusWithCurrentRuleFile_ResolvesCompletelyAndStaysStable`, which folds this row, row 34, and row 35 into one test — see that test's own remarks for why one test now covers all three |
| 34 | ✅ | The cold-start half still passes, with the known conflicts accounted for | Unit test | **Corrected 2026-09-07/08:** folded into `NikhilNamal17RealCorpusWithCurrentRuleFile_ResolvesCompletelyAndStaysStable` (row 33) — per the developer's directive that a feature test must use a synthetic fixture, this is now the one deliberate exception allowed to depend on the real bundled corpus, so the three previously-separate real-corpus tests (this row, row 33, row 35) are one test, not three independently-drifting ones |
| 35 | ✅ | The two existing real-rule-file tests still pass | Unit test | **Corrected 2026-09-07/08:** `InitialiseAsync_NikhilNamal17WithRealRuleFile_ProducesNoUnresolvedActions` was renamed and extended into `NikhilNamal17RealCorpusWithCurrentRuleFile_ResolvesCompletelyAndStaysStable` (row 33) — `KnownUnresolvedNikhilNamal17QuoteIds` is now an empty set, not four documented exceptions, since every one of them resolves cleanly |
...
| 43 | ✅ | The behaviour is correct on the developer's own machine | Live (T1) | Confirmed by the developer 2026-09-08, Visual Studio, against live-fetched source data (not just the static bundled copy): two consecutive reseeds each report `pending=0 stale=0 blocked=0 discarded=0` across every entity type in every file, with only the expected "Lion King" two-real-films self-contradiction warning remaining |
...
| 53 | ✅ | A `Stale` conflict does not double on every reseed | Unit test | `DatabaseInitializerTests.Reseed_Repeatedly_WithAStaleRuleConflict_StaleCountNeverGrows` (minimal reproduction) and the consolidated `NikhilNamal17RealCorpusWithCurrentRuleFile_ResolvesCompletelyAndStaysStable` (real corpus, row 33) — step 12's seventh live-found defect, found by the developer's own T1 run. **Corrected 2026-09-07/08:** the minimal reproduction's fixture was changed from a recorded snapshot that matched one real raw occurrence (`"1991"`) to one that can never match any real value (`"9999"`) — the original shape stopped reproducing Stale at all once [#378](https://github.com/DutchJaFO/Quotinator/issues/378) was fixed (a Keep/Replace decision now correctly controls which Source variant a quote links to), so the fixture needed to become a genuinely, permanently unresolvable conflict, independent of #378, to keep proving the accumulation-dedup mechanism this row exists for |
| 54 | ✅ | A Keep/Replace decision on a Quote's `date` field actually controls which Source variant the quote links to, not just its reported field | Unit test | Root-caused and fixed as its own issue, [#378](https://github.com/DutchJaFO/Quotinator/issues/378) — `DatabaseInitializerTests.Seed_WithADuplicateQuoteKeepRule_QuoteLinksToTheKeptDateNotTheLastOccurrences`, confirmed red without the fix (`git stash` of `ImportActionPlanner.cs`) and green with it. This is what makes row 33's genuine zero achievable via `Keep` (Auntie Mame) rather than only via `Custom` |
