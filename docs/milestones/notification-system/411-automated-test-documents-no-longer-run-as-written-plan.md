...
**Two application findings, outside this issue** — filed as #419 and #421:
...
| *A notification's title and body resolve to the requested language* | Pass — every value `True` in steps 2–5 and 7, `400` for the malformed language; the log read added before the cleanup's stop finds nothing |
| *Upgrading translates the notification a released build already wrote* | Pass after a fix — folder cleared; reads before its three stops. The cleanup read now counts from after step 6's restart, whose own two shutdown `SocketException`s a cumulative count reported as thrown (`2`, both `Socke…` on the restart's second). `1`/`0` before, all four `True`, `1` after, `de 1`/`nl 1`, `new=0` |
| *A reset recommends a reseed, and running the recommendation resolves it* | Pass — `795`/`0`, `0`/`1` `actionrequired`, `795`/`0`, `resolved`, `1`; the cleanup read added finds nothing |
| *A reseed confirms each file that applied cleanly, once per result* | Pass after four fixes. Its `Fully green after: #372 and #373` header no longer applied (step 2: `resolvedToExisting` 24, every row adds up) and is removed. Step 3 forbade `added=0 modified=0`, which since #373 is every touched type on an unchanged reseed; it now forbids `incoming=0` (none of 21 rows). Step 7's variants counted cold start's confirmations too (`11b` 10, `11c` 2) and now dismiss them first, gated on the ready line (`11a` logs no `seeding complete`): `0`/`5`/`1 User`/`6`. Step 8 omitted `quotinator-seasons.json`; reads added before its restarts; the cleanup removed a folder nothing creates |
| *A notification renders its title and body as separate things…* | **Rewritten into a runnable order.** Step 1's untitled fixture wrote through a `$bind` only step 6 defined, on a container never given `--bind`; steps 9–10 assumed an actionable row and a review alert nothing created. It now seeds the conflict fixture on its own bind, reads the page before the modal's restart, and drops a guessed five-second sleep. Every step passes: one title element, one untitled row, `pre-line` `4 > 2`, a seven-row dialog at `420` high (`364`), `Decide` asking with `1` pending, *Took the values from the file* with `0`, the modal's 11 expanders, degraded `503/500/200`. Two stale expectations: the table has eight headings, not three, and its `95vh` threshold failed at `1274` high (`1218` > `1210.3`) — the cap is Bootstrap's `100% − 3.5rem`, since `.modal-dialog-centered`'s `min-height` overrides the inline `max-height: 95vh`; it now asserts fit to the viewport. Filed as #422 |
| *The changelog is served from its own on-disk database* | Pass after a fix — folder cleared; reads before its restart and its stop, the latter from after the restart. Both files on disk; `126` refreshed and served; `/about` 43 entries, served `3 → 4`; `0` fallbacks; `126`/`126` across the restart; `new=0` |
| *Seeding takes a backup, a broken schema degrades rather than crashes, and Reset recovers* | Pass after a fix — folder cleared, reads before each stop. Step 3 waited `--wait-for 503`, which the startup wait page (#280) also answers: measured, it returned at once, health read `starting`, `/quotes/random` gave the wait page's `200`. It now polls `/health` for `unhealthy`, reading a `503`'s body from the response stream since PowerShell 5.1 leaves `ErrorDetails.Message` empty (a first attempt ran its full 120 s for that reason). Third run: `down,starting` then `unhealthy` at 3 s; backup → seeding failed → restored → initialisation failed; `backups=1`, `Up`; `unhealthy`/`unavailable`; `401 401`; zeros; `healthy`, `NoResults`; `new=0` |
| *Startup backs up only when there is real work, and respects the storage budget* | Pass after a fix — backups `0, 0, 1, 2`, reads before each stop clean. Step 5 expected Reset to succeed with the backup skipped; since the full-quota work it refuses: `409` `BudgetExceeded` with remedies, `backups=2`, `795` quotes still present. Step 5 now asserts the refusal. Step 3's reset straight after a restart did not race the what's-new write — after a restart that row already exists |
| *A recorded schema version ahead of the build stays healthy and says so* | Pass after a fix — folder cleared, reads before both stops clean. Healthy `200`, the notification `actionrequired`, the log line `recorded app v10 (known: v9)`. Step 4 matched `database Reset`, which the reworded body (*Reset the database from the admin endpoints…*) no longer contains (`False` on a body that names the remedy); it now matches the current phrase. Still healthy on this build — #350's reversal has not landed |
...
| 12 | ✅ | Every document whose environment changed still passes | Live (T2) | Steps 14 and 15: every document in the suite passes as written, or declares the issue it waits on — 67 of 68 pass; *Rule-file override endpoints* fails on the `generate` defect and declares `Fully green after: #420` |
