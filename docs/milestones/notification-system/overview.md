...
| [#375](https://github.com/DutchJaFO/Quotinator/issues/375) | A quote from a multi-season TV series cannot say which season it is from | Planning | T1 ⬜ T2 ⬜ | — |
...
#375 ─── depends on nothing, and blocks #374. Filed 2026-09-03 out of #374's planning. A quote has no
         ...
         show has nowhere to say so, and four bundled `tv` titles already carry such quotes. Where a
         season belongs in Universe → Series → Source is a trade-off ADR 011 does not settle, so this
         issue owns an ADR; if the answer makes Season an entity, it also owns the masterdata endpoint
         pair and the pagination contract that comes with one
