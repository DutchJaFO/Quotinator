...
| [#367](https://github.com/DutchJaFO/Quotinator/issues/367) | Notification actions give no feedback while they run | In progress | T1 ⬜ T2 ⬜ | [367-executing-notification-state-plan.md](367-executing-notification-state-plan.md) |
...
| 16 | **#367** | **Moved up from 31 (developer, 2026-09-01).** An executing state for a running notification action. Ahead of #308 rather than after it: a new status label is a layout decision, and #308 settles per-type layout across both surfaces — deciding the status first means #308 designs against the finished set instead of revisiting it |
...
| 31 | **#369** | Orphaned review rows, found in #303's T1. Straight after #303 rather than at the end: it is a correction to a page that just shipped, and every extra reseed run between now and then adds more undecidable rows to whatever database is being used to verify the issues above it |
| 32 | **#370** | Expected conflicts signalled by throwing. Straight after #369 so one T2 pass covers both `/import-review` changes, and because #369 changes which rows that page renders — settling that first means this issue's "zero throws for N rows" count is measured against the row set the page will actually keep |
