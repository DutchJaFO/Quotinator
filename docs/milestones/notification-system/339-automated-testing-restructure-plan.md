...
| 25 | ❌ | Every document can distinguish the feature working from the feature broken | Live | 13 repaired by adding the observation that was missing. **`16` and `17` cannot, and are therefore failing tests that block release.** The seed logs a per-file report rendering `stale=0`, so "the reseed ran" is observable; whether the staleness *evaluation* ran is not, because `stale=0` is produced identically by *compared the rules, none drifted* and by *never compared anything*. Clearing them means emitting that evaluation, not constructing a fixture to force the mechanism to fire. The row's original wording — "repaired, *or* its limit is stated" — was wrong: stating a limitation explains a failing test, it does not resolve one |
