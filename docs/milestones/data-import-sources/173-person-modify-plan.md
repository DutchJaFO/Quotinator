...
| 10 | ❌ | `ClearStaleAddTargetsAsync` correctly hard-deletes a stale Person Add whose explicit id is lowercase | Unit test | `Quotinator.Engine.Tests.ClearStaleAddTargetsAsync_PersonExplicitLowercaseId_HardDeletesCorrectly` |
...
| 13 | ❌ | Live: a `people[]` correction to a Person's `name`/`dateOfBirth`/`dateOfDeath` stages/decides/applies a `Modify`, and a `Complete` Person's field cannot be silently overwritten | Live (T2) | Docker smoke test against `docker build -f docker/Dockerfile -t quotinator:local .`: import a file with an explicit `people[]` entry (new `dateOfBirth`/`dateOfDeath` values) under `review` policy, confirm `GET /import/actions?status=pending` shows a `Modify` action with the field diff, `decide` + `apply` it and confirm the Person row changed (via `Quotinator.Tools.DbInspector` or a quote lookup exposing `author`/dates); separately, mark a Person `Complete` via `markCompletenessAs`, re-import a changed field for the same id, and confirm the resulting action is `Blocked` (`GET /import/actions?status=Blocked`), not silently applied; finally, stage/apply/reverse a lowercase-id `people[]` entry end to end to confirm `ClearStaleAddTargetsAsync`'s fix (requirement 10) holds live, not only under the unit test's SQLite fixture |
