...
**Three further gaps found from a manual review of the running app after T1/T2** (user-reported, not part of the original verification pass):

1. **The new endpoints were invisible in the Blazor "REST API reference" page.** That page's endpoint tables are hand-written HTML, not generated — added a new "Protected quote endpoints" section (own heading/description/table, gated behind the same `AdminKeyConfigured` check as the existing admin table, since these endpoints share the identical `X-Api-Key` requirement despite not being under `/api/v1/admin`). New `ApiRoutes.QuotesImport`/`QuotesImportPreview` constants; new i18n keys in all three locales.
2. **`ImportResultResponse` (and `ImportSummary`/`ImportConflictEntry`/`ImportRowError`) never appeared in the OpenAPI spec's `components/schemas`**, because neither endpoint declared `.Produces<T>()` and the handler returns generic `Task<IResult>` — the generator had no way to infer the response type. Fixed by adding `.Produces<ImportResultResponse>(200)` plus the actual error status codes (`401`/`422`/`501`) to both routes. Confirmed live: all four types now appear in `/openapi/v1.json`'s `components/schemas` with full property/required-field detail from their XML doc comments.
3. **The `settings` field's inline `JsonSerializer.Deserialize` + `try`/`catch` in the endpoint handler was replaced with a dedicated `ImportRequestSettingsParser.TryParse`** (`Quotinator.Data.Import`, mirroring `ConflictPolicyParser`'s existing precedent) — not a raw-JSON exception, since a POCO (`ImportRequestSettingsDto`) already existed for it; this was flagged as a policy question ("never use raw JSON when we have a POCO") and reconciled explicitly: binding `settings` as a real framework-typed parameter (via `IParsable`/`TryParse` conventions) was considered but rejected, because ASP.NET Core's built-in binding-failure path can't produce the existing localised `422` — it either forces a generic `400`, or (via `BindAsync`) silently treats malformed input the same as "omitted", masking a real user error. The parser centralises the JSON handling into one small, independently unit-tested seam (`ImportRequestSettingsParserTests`, 7 tests) while keeping the custom `422` path; the `settings` field's shape remains documented via `[Description]` prose only, not a wired OpenAPI request schema — an accepted, known limitation of OpenAPI 3.x's multipart modeling for "one field containing a JSON string" that's out of scope for this issue.

**Full solution `dotnet test --configuration Release`: 821 tests, 0 warnings, 0 errors.** `dotnet build --configuration Release`: 0 warnings, 0 errors.
