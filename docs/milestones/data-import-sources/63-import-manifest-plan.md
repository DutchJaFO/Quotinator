...
| T1 | ✅ | App starts cleanly in Visual Studio; imports-dir manifest auto-created with `[Database - Init]` warning logged | Live (VS) | Confirmed 2026-06-30 — schema v4, 788 quotes seeded, `imports\manifest.json` auto-created with correct warning logged via Serilog |
| T2 | ✅ | `docker build`/`docker run` smoke test; auto-creation works inside the container | Live (Docker) | Confirmed 2026-06-30 — `docker build` succeeds; `/api/v1/health`, `/api/v1/version`, `/api/v1/quotes/random` all return expected output; manifest auto-create verified inside the running container at `/app/data/imports/manifest.json` with warning logged |
