...
| 7 | ✅ | Existing import behavior preserved by the planner/applier extraction, with preview's contract intentionally changed (Key decision 3) | Unit test + Live | `QuoteImportServiceTests` — 21/21 pass; 2 tests rewritten for the new preview contract, not left unmodified. Live: `POST /import` with a file containing a single brand-new quote/source/character (guaranteed zero conflicts by construction) against a real `dotnet run` server returned `200 OK` with `summary.total=1, imported=1, updated=0, skipped=0, errors=0` and `conflicts: []` — the exact shape a clean, non-ambiguous import always had. `GET /quotes/{id}` and search-by-source both confirmed the quote, Source, and Character landed; `GET /import/actions?batchId=` showed all three staged actions (`Source`/`Character`/`Quote`, all `Add`) as `Applied`, proving the planner/applier extraction reproduces the pre-#154 zero-conflict import path end-to-end, not just in the two rewritten unit tests |
