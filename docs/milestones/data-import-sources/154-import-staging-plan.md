...
| 7 | ✅ | Existing import behavior preserved by the planner/applier extraction, with preview's contract intentionally changed (Key decision 3); zero-conflict and resolved-but-non-pending-conflict files must map to `200`, only a genuinely pending conflict to `202` | Unit test + Live | `QuoteImportServiceTests` — 21/21 pass; 2 tests rewritten for the new preview contract, not left unmodified. The manual live check (a file with a single brand-new quote/source/character against a real `dotnet run` server, confirming `200`/`conflicts: []`/all staged actions `Applied`) was formalised into automated tests rather than left as a one-off: `ImportAsync_FreshDatabase_InsertsNewQuote` now asserts `Conflicts.Count == 0` directly (Engine layer, real SQLite, real duplicate detection); `ImportAsync_NewestWins_ReplacesExistingRow` now asserts a real auto-resolved conflict has `Status == "resolved"`, proving a conflict can exist without blocking; `ImportAsync_Review_BehavesLikeSkip` (pre-existing) already covers a genuine ambiguous conflict producing `Status == "pending"`. At the endpoint layer (`ImportEndpointTests`, fake service — status-code mapping is deliberately isolated from real duplicate detection there): `Import_CorrectKeyAndValidFile_Returns200WithResultShape` now asserts the JSON `conflicts` array is empty; new `Import_ResultHasOnlyResolvedConflicts_Returns200NotAccepted` (parameterised over `/import` and `/import/preview`) proves a **non-empty but fully-resolved** conflicts list still maps to `200`, not `202` — red/green verified by temporarily changing `ToStatusCodeResult`'s check from "any pending" to "any conflicts at all" and confirming this new test catches the regression. Full suite: 942/942 |
