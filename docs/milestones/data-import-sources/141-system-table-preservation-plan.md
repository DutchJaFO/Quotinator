...
**Status:** Fully verified 2026-07-03 — the original hardcoded-exclusion-list mechanism (below, "Scope narrowed") was replaced with a `System_`-prefix naming convention before release, per the user's design intent. `Quotinator.Data` now identifies protected tables generically via `Sql.Schema.GetUserTables`'s escaped `LIKE 'System\_%' ESCAPE '\'` pattern match — it never needs to know specific table names. `AuditEntries`/`SchemaVersion` are renamed to `System_AuditEntries`/`System_SchemaVersion` at both the SQL and C# class level. **T1 verified live 2026-07-02** by the user in Visual Studio, including the migration004/migration006 Reset collision the fix targeted at the time — see row 22. That collision-avoidance mechanism (`IsKnownMigrationError`) was later removed entirely under #143's exception-free-migrations amendment (2026-07-03), which fixes the same underlying problem at the root instead (Reset no longer wipes/replays `System_SchemaVersion` at all, so the collision is structurally impossible) — see #143's plan doc for detail; #141's own requirements (System-table survival, `preserveSchemaVersion` behaviour) are unaffected by that later change. **T2 verified 2026-07-03** — see row 23.
**GitHub issue:** #141
