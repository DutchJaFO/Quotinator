...
#154 is `In progress` — all of Phase A's implementation is done (stable ids, the generic Data-layer staging primitive, the Engine-side planner/applier, import-path and seeding rewiring, the `ImportBatch` enum fix, and the new `/import/actions/*` endpoints with the `200`/`202` status-code split on `/import`/`/import/preview`), verified against a live dev server as well as the full unit/integration suite (976 tests, 0 failures). What remains is the Phase A gate itself (a full T1/T2 pass — Visual Studio and Docker, not yet done) and Phase B (deleting #149's now-superseded `System_ImportConflicts` machinery once that gate passes). All other remaining issues (#59, #67, #68, #69, #144, #153, #155) are still in `Planning` — not started. #149 and #152 are `Waiting for release` (both: T1 ✅ T2 ✅). Evaluate each for early merge when complete — the default is to merge the full milestone together.
