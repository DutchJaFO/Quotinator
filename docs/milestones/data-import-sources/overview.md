...
#154 is `In progress` — all of Phase A's implementation is done (stable ids, the generic Data-layer staging primitive, the Engine-side planner/applier, import-path and seeding rewiring, the `ImportBatch` enum fix, and the new `/import/actions/*` endpoints with the `200`/`202` status-code split on `/import`/`/import/preview`). The Phase A gate is substantially done: every scenario in the plan doc's verification table has been exercised live — a fresh-database startup with a genuinely ambiguous seed file (staged, not blocking, absent from `GET /quotes` until applied), the full decide→apply cycle, stable-id idempotency (confirmed via direct DB inspection), and a full Docker build + container run reproducing the same behaviour (T2 ✅). The developer's own VS pass (T1) caught a real bug my own `dotnet run` testing missed — `GET /import/actions?batchId=`/`apply`/`discard` did a case-sensitive comparison against a value the live `/import` response returns lowercase but the table stores uppercase, so a batch id copied straight from `/import`'s response silently matched nothing (worst case: `apply` would report `200 OK` having applied zero actions). Fixed with a case-insensitive SQL comparison, red/green verified, full suite 977/977. The same VS pass also surfaced that `POST /import`'s `batchId`-mode alias (apply an already-staged batch through the same route file-mode uses) hadn't actually been built — the developer confirmed this was a required, not optional, part of the staging→execution design, so it was added as a same-session follow-up (`IQuoteImportService.ApplyStagedBatchAsync`, `ImportBatchNotFoundException`, nullable `IFormFile? file`), full suite green. The developer then flagged that the original casing fix was too narrowly scoped — the unit tests hadn't caught it, and any query parameter carrying a GUID or enum-like value should match case-invariantly by default, not just the one reported instance. Auditing on that principle found the identical unfixed bug in `status`/`entityType` on `/import/actions` and in `batchId`/`status` on the still-live legacy `/import/conflicts` table; all four were fixed together (`UPPER(col) = UPPER(@param)`), with new case-variant regression tests, red/green verified. Full suite: 988/988 across all 9 test projects. The one remaining open item is that this VS pass wasn't a *complete* T1 run through every scenario in the plan doc's table — held at 🟡 pending the developer confirming the rest before Phase B (deleting #149's now-superseded `System_ImportConflicts` machinery) proceeds. All other remaining issues (#59, #67, #68, #69, #144, #153, #155) are still in `Planning` — not started. #149 and #152 are `Waiting for release` (both: T1 ✅ T2 ✅). Evaluate each for early merge when complete — the default is to merge the full milestone together.
