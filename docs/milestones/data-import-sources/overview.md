...
| [#141](https://github.com/DutchJaFO/Quotinator/issues/141) | Reseed/reset must preserve System-classified data | 🟡 Code complete, T1 verified — T2 blocked in this session (scope narrowed, see plan doc) | T1 ✅ T2 ⚠️ blocked | [141-system-table-preservation-plan.md](141-system-table-preservation-plan.md) |
...
**Shipped in:** (next release)
T1 and T2 required — corrected 2026-07-02: this changes the actual table-wipe logic behind `Reset` (`DropAndRebuildAsync`, `Sql.Schema.GetUserTables`), and `docs/release-verification.md` now explicitly calls out schema/reset-logic changes as a T1/T2 trigger regardless of whether `.razor`/Dockerfile/`Program.cs` are touched. Unit tests already caught one real bug during this work (an early `preserveSchemaVersion:true` design silently dropped every data table without recreating them).

**T1 ✅ verified 2026-07-02** — `dotnet run` against a real, persistent database (not a unit-test temp file). Baseline: 6 `AuditEntries` rows, `SchemaVersion` all timestamped identically, 788 quotes. Default reset: `AuditEntries` grew to 13 (original 6 survived), `SchemaVersion` timestamps all refreshed (unchanged historical behaviour confirmed), 788 quotes rebuilt. `preserveSchemaVersion=true` reset immediately after: `SchemaVersion` timestamps stayed exactly as the prior reset left them (preservation confirmed), `AuditEntries` kept growing (13→20), quotes rebuilt again, health/version/random endpoints all correct. Checked via `Quotinator.Tools.DbInspector` against the real `.db` file.

**T2 ⚠️ blocked in this session** — the Docker daemon cannot start in this sandboxed cloud environment (`ulimit: error setting limit (Operation not permitted)`, confirmed even with the sandbox override). Not a code gap — T1 already exercises the identical code path T2 would exercise in a container. Needs completion on a local machine or via CI before this issue can close. T3 still does not apply.
...
| 7a | #141 | Reseed/reset must preserve System-classified data | Resolved in code 2026-07-02 — scoped to `AuditEntries`/`SchemaVersion` only after cross-check with the user; T1 verified live, **T2 blocked** (no Docker daemon in this session — needs local/CI), pending release |
