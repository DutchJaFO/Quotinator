...
#154 is `In progress` (implementation complete, T1 ✅ T2 ✅) — not yet closed, pending explicit developer confirmation. Phase A: stable ids, the generic Data-layer staging primitive, the Engine-side planner/applier, import-path and seeding rewiring, the `ImportBatch` enum fix, and the new `/import/actions/*` endpoints with the `200`/`202` status-code split on `/import`/`/import/preview` — every scenario in the plan doc's verification table exercised live (fresh-database startup with a genuinely ambiguous seed file, the full decide→apply cycle, stable-id idempotency, T2 ✅). The developer's own VS pass (T1) caught a real bug my own `dotnet run` testing missed — a case-sensitive `batchId` comparison that silently matched nothing when a caller round-tripped the lowercase id `/import`'s response returns — fixed, red/green verified. The same VS pass also surfaced that `POST /import`'s `batchId`-mode alias hadn't actually been built despite being a required part of the staging→execution design; built as a same-session follow-up. The developer then flagged the casing fix was too narrowly scoped — any query parameter carrying a GUID or enum-like value should match case-invariantly by default — and auditing on that principle found and fixed the identical unfixed bug in `status`/`entityType` on `/import/actions` and `batchId`/`status` on the legacy `/import/conflicts` table, with new regression tests, red/green verified. Full suite at that point: 988/988. **Phase B**: with the developer confirming their T1 pass complete, `/import/conflicts/*` and all of #149's superseded machinery were deleted in one focused pass — 20 whole files removed (entity, coordinator, service, repositories, response DTOs, route registrations, test doubles, and all five #149-specific test files) plus dead supporting code (`QuoteSeedWriter.LogImportConflictAsync`, the `ImportConflictStatus` enum, `ApiRoutes`/`ApiMessages` constants, i18n keys in all three locales, README/addon-DOCS/RestApi.razor rows). A stale incremental build initially masked 6 dangling XML-doc `<see cref>` warnings in surviving files — caught and fixed only after a genuinely clean rebuild (`dotnet clean` + `build`), landing at 0/0. Full suite: 938/938 across all 9 test projects. Live smoke test against the built binaries confirmed `/import/conflicts` now returns `404` while `/import/actions` and other endpoints are unaffected. The `System_ImportConflicts` migration/baseline history and its schema-drift test were left untouched, per the plan — squashing that history is #155's call, not this issue's. **T2**: a full Docker build + container cycle covering both phases — clean fresh-seed startup, `/import/conflicts` confirmed `404`, the full decide→apply→discard cycle, the `batchId`-mode alias, and case-insensitive query filters all verified live in the container, no errors. That pass surfaced one more real bug: a genuinely bodyless `POST /import` request (no `Content-Type`, no body, no `batchId`) returned a bare, uninformative framework `400` under real Kestrel — bypassing `BadRequestExceptionHandler` entirely, since Minimal API's automatic `IFormFile?`/`[FromForm]` binding fails at the routing layer itself for a request with no form content-type at all, not via a catchable exception. Fixed by reading the request manually (`HttpRequest`, checking `HasFormContentType`) instead of relying on automatic binding, returning a clear `422` ("you must provide either a file or a batchId") in that case — verified against a real `dotnet run` Kestrel instance, not just the in-memory TestServer, since the old broken behavior differed between the two hosts. Full suite: 940/940. The fix was then re-verified in a rebuilt Docker container too, confirming the exact scenario that had returned a bare `400` now returns `422`/`404` correctly. The scattered, drifted copies of the T2 smoke-test list found across `docs/release-verification.md`, `docs/ci-cd.md`, and `docs/workflow/release.md` were consolidated into one authoritative, living checklist in `CLAUDE.md`'s Pre-Push Checklist — mandatory whenever a T2 pass runs, expanded whenever a new scenario is found. All other remaining issues (#59, #67, #68, #69, #144, #153, #155) are still in `Planning` — not started. #149 and #152 are `Waiting for release` (both: T1 ✅ T2 ✅). Evaluate each for early merge when complete — the default is to merge the full milestone together.
