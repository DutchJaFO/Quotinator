...
#207 (canonicalize file-authored explicit ids at capture) → found live via T2 during #190 (2026-07-19): a lowercase-authored explicit Source id imports and reads correctly via the Quote→Source join, but 404s via the masterdata Sources `GetById` endpoint, because `Sources.Id`/`Quotes.SourceId`/`CharacterSources.SourceId` are all written from the same uncanonicalized `ImportActionPlanner` in-memory value — accidentally self-consistent with each other, never actually canonical. Formalised as ADR 012, which requires the fix to go through a single reusable `Quotinator.Data.Helpers.EntityIdCanonicalizer` helper plus a cross-entity `[DynamicData]` guard test, not an ad hoc per-call-site fix — a narrower fix (uppercase only the Source insert) was considered and rejected after tracing that it would break the Quote→Source join outright. Independent of #190 itself (a data-consistency bug, not an absent-vs-null bug), though found during its T2 pass. Scope expanded during planning (2026-07-19): `Quotes.Id` has the identical capture-time gap, targeting lowercase (per `QuoteIdentity.StableId`'s own pinned convention) rather than uppercase — and `Sql.Quotes.SelectById()` (the query behind `GET /quotes/{id}`) has no case-insensitive wrapping at all today, unlike Source/People's partial mitigation. Folded in as "Part B" rather than filed separately, per explicit developer decision.
...
| 57 | #163 | Bulk-decide a staged import batch via file export/import, CSV and JSON (Phase 1 of #153) | Planning |
| 58 | #181 | Minimal per-source conflict-resolution rule file + curated field-override preload | Planning |
| 59 | #153 | Declarative conflict-resolution file for recurring third-party source conflicts (Phase 2) | Planning |
| 60 | #177 | ImportBatches.Status never set to Applied via staged apply, breaking reversal | Planning |
| 61 | #155 | Migration review: verify full incremental path from last-shipped v1.7.2 schema | Planning |
