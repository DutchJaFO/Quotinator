...
| 14 | `audienceHighlights` field and `--audience` generator parameter | Each release entry may carry an `audienceHighlights` object whose keys are audience names (e.g. `ha-addon`, `customer`). The `ha-addon` format checks for `audienceHighlights.<audience>` before falling back to `highlights`. `--audience` (default: `ha-addon`) makes the key configurable so a single `changelog.json` can produce tailored output for different audiences without forking the file. |
| 15 | `--fallback false` / `--fallback-message` generator parameters | When a release has no content for the requested audience/highlights, the default is to omit the section. `--fallback false` replaces that omission with a configurable sentence (default: `"No user-facing changes."`). Applies to both `keepachangelog` (empty `highlights`) and `ha-addon` (absent audience key). The distinction matters: with `--fallback false --audience customer`, an entry missing `audienceHighlights.customer` emits the message rather than leaking standard `highlights` to that audience. |
| 16 | `unreleased` section in schema, generator, and import script | The schema defines an `unreleased` object at the top level (fields: `issues`, `cves`, `highlights`, `added`, `changed`, `fixed`, `removed`) mirroring the structure of a release entry but without `version` or `date`. The generator renders it as `## [Unreleased]` in `keepachangelog` format and omits it from `ha-addon` (the HA Store shows only released versions). The import script maps a `[Unreleased]` block to the `unreleased` key. The `unreleased.issues` field is intentionally not auto-detected from prose — must be set manually. |
...
- Issue has an `internal: true` boolean on entries to suppress highlights for internal releases — replaced by `--fallback false`/`--fallback-message` generator parameters, which give the caller control over the output without requiring a per-entry flag in the data
- Issue does not mention an `unreleased` section — added here following the Keep a Changelog `[Unreleased]` convention; schema mirrors release entry fields minus `version` and `date`
- Issue does not mention `audienceHighlights` or `--audience` — added here to enable audience-specific highlight output without forking `changelog.json`
