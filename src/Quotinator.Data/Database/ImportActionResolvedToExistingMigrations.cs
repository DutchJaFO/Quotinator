...
/// <summary>
/// #377: widens <c>Import_Action.ActionType</c>'s CHECK constraint to admit <c>ResolvedToExisting</c>,
/// the outcome for a record whose fields differed from what arrived but whose resolution settled on the
/// values already stored. Consumed by <see cref="DatabaseInitializer.DataOwnedMigrations"/>, which owns
/// the version number.
/// </summary>
/// <remarks>
/// A full table rebuild, because SQLite cannot widen an inline CHECK constraint — the same technique
/// <see cref="ImportActionUnchangedMigrations"/> used on this table for #373, and the shape ADR 008
/// requires whenever an enum-backed column gains a member.
/// <para>
...
/// </para>
/// </remarks>
public static class ImportActionResolvedToExistingMigrations
{
    /// <summary>Rebuilds <c>Import_Action</c> with <c>ResolvedToExisting</c> accepted by <c>ActionType</c>.</summary>
    public const string WidenActionTypeForResolvedToExisting = """
