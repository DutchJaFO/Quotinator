...
/// <summary>
/// #376: widens <c>Import_Action.ActionType</c>'s CHECK constraint to admit <c>AlreadyReported</c>, the
/// outcome for a record whose conflict an earlier pass already staged and which is therefore not staged
/// again. Consumed by <see cref="DatabaseInitializer.DataOwnedMigrations"/>, which owns the version
/// number.
/// </summary>
/// <remarks>
/// A full table rebuild, because SQLite cannot widen an inline CHECK constraint — the same technique
/// <see cref="ImportActionUnchangedMigrations"/> (#373) and <see cref="ImportActionResolvedToExistingMigrations"/>
/// (#377) each used on this table, and the shape ADR 008 requires whenever an enum-backed column gains a
/// member.
/// <para>
/// The copy is a straight column-for-column carry: nothing about existing rows changes, and no value is
/// rewritten. Only the constraint admits one more member, so every row valid before is valid after.
/// </para>
/// </remarks>
public static class ImportActionAlreadyReportedMigrations
{
    /// <summary>Rebuilds <c>Import_Action</c> with <c>AlreadyReported</c> accepted by <c>ActionType</c>.</summary>
    public const string WidenActionTypeForAlreadyReported = """
