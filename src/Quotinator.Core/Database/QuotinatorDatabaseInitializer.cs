    ...
    SchemaBaseline? baseline = null) : DatabaseInitializer(factory, options, migrations, auditWriter, callerContext, logger, diskSpaceProvider, baseline)
{
    ...
    internal static string FormatReport(FileImportReport report)
        => string.Join(" ", report.EntityTypes.Select(kv =>
            $"{kv.Key}[incoming={kv.Value.Incoming} new={kv.Value.New} unchanged={kv.Value.Unchanged} resolvedToExisting={kv.Value.ResolvedToExisting} alreadyReported={kv.Value.AlreadyReported} skipped={kv.Value.Skipped} modified={kv.Value.Modified} blocked={kv.Value.Blocked} discarded={kv.Value.Discarded} pending={kv.Value.Pending} stale={kv.Value.Stale}]"));
