...
internal static class Sql
{
    ...
    internal static class Quotes
    {
        ...
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_Quote SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string UpdateOnNewestWins =
            $"UPDATE Quotinator_Quote SET QuoteText=@text, OriginalLanguage=@lang, SourceId=@sid, " +
        ...
        private static readonly string SelectBase = $"""
            ...
            FROM Quotinator_Quote          q
            JOIN Quotinator_Source         s  ON  {IdClauses.Join("s.Id", "q.SourceId")}                                          AND s.IsDeleted  = 0
            LEFT JOIN Quotinator_Character   c  ON  {IdClauses.Join("c.Id", "q.CharacterId")}                                       AND c.IsDeleted  = 0
            LEFT JOIN Quotinator_Person       p  ON  {IdClauses.Join("p.Id", "q.PersonId")}                                          AND p.IsDeleted  = 0
            LEFT JOIN Quotinator_QuoteTranslation    qt ON {IdClauses.Join("qt.QuoteId", "q.Id")} AND {TextClauses.Equals("qt.Language", "lang")} AND qt.IsDeleted = 0
            LEFT JOIN Quotinator_SourceTranslation   st ON {IdClauses.Join("st.SourceId", "s.Id")} AND {TextClauses.Equals("st.Language", "lang")} AND st.IsDeleted = 0
            LEFT JOIN Quotinator_CharacterTranslation ct ON {IdClauses.Join("ct.CharacterId", "c.Id")} AND {TextClauses.Equals("ct.Language", "lang")} AND ct.IsDeleted = 0
            LEFT JOIN Quotinator_Series       ser ON {IdClauses.Join("ser.Id", "s.SeriesId")}                                         AND ser.IsDeleted = 0
            LEFT JOIN Quotinator_Universe     uni ON {IdClauses.Join("uni.Id", "ser.UniverseId")}                                     AND uni.IsDeleted = 0
            """;
        ...
        private static readonly string CountForRandomBase =
            "SELECT COUNT(*) FROM Quotinator_Quote q " +
            $"JOIN Quotinator_Source s ON {IdClauses.Join("s.Id", "q.SourceId")} AND s.IsDeleted = 0 " +
            $"LEFT JOIN Quotinator_Character c ON {IdClauses.Join("c.Id", "q.CharacterId")} AND c.IsDeleted = 0 " +
            $"LEFT JOIN Quotinator_Person p ON {IdClauses.Join("p.Id", "q.PersonId")} AND p.IsDeleted = 0";
        ...
        private static readonly string CountForGetAllBase =
            $"SELECT COUNT(*) FROM Quotinator_Quote q JOIN Quotinator_Source s ON {IdClauses.Join("s.Id", "q.SourceId")} AND s.IsDeleted = 0";
    ...
    internal static class QuoteGenres
    {
        ...
        internal const string Insert =
            "INSERT OR IGNORE INTO Quotinator_QuoteGenre " +
        ...
        internal static readonly string InsertWithExistsGuard =
            ...
            $"WHERE EXISTS (SELECT 1 FROM Quotinator_Quote WHERE {IdClauses.Equals("Id", "QuoteId")} AND IsDeleted = 0);";
    }
    ...
    internal static class QuoteTranslations
    {
        ...
        internal const string Insert =
            "INSERT OR IGNORE INTO Quotinator_QuoteTranslation " +
    ...
    internal static class SourceTranslations
    {
        ...
        internal static readonly string CountForSource =
            $"SELECT COUNT(*) FROM Quotinator_SourceTranslation WHERE {IdClauses.Equals("SourceId", "sid")} AND {TextClauses.Equals("Language", "lang")} AND IsDeleted = 0;";
    }
    ...
    internal static class Characters
    {
        ...
        internal static readonly string SelectGlobalCandidateId =
            $"SELECT {IdClauses.SelectColumn("c.Id", "Id")} FROM Quotinator_Character c " +
            $"JOIN Quotinator_CharacterSource cs ON {IdClauses.Join("cs.CharacterId", "c.Id")} AND cs.IsDeleted = 0 " +
            $"JOIN Quotinator_Source s2 ON {IdClauses.Join("s2.Id", "cs.SourceId")} " +
        ...
        internal static readonly string CountActiveReferences =
            $"SELECT COUNT(*) FROM Quotinator_Quote WHERE {IdClauses.Equals("CharacterId", "id")} AND IsDeleted = 0;";
        ...
        internal const string InsertIfNotExists =
            "INSERT OR IGNORE INTO Quotinator_Character (Id, Name, SourceType, ImportBatchId, DateCreated, DateModified, DateDeleted, IsDeleted, CompletenessStatus, NoValueKnown) " +
        ...
        internal static readonly string SelectExistingById =
            $"SELECT Name, CompletenessStatus FROM Quotinator_Character WHERE {IdClauses.Equals("Id", "id")} AND IsDeleted = 0;";
        ...
        internal static readonly string SelectCompletenessById =
            $"SELECT CompletenessStatus, NoValueKnown FROM Quotinator_Character WHERE {IdClauses.Equals("Id", "id")};";

        /// <summary>Persists #165's decide-time override or auto-computed transition — the only path allowed to change CompletenessStatus after insert. Case-insensitive — see <see cref="Sources.SelectExistingById"/>'s remark.</summary>
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_Character SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string UpdateFieldsById =
            $"UPDATE Quotinator_Character SET Name = @name, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
    }
    ...
    internal static class CharacterSources
    {
        ...
        internal const string InsertIfNotExists =
            "INSERT OR IGNORE INTO Quotinator_CharacterSource (Id, CharacterId, SourceId, DateCreated, DateModified, DateDeleted, IsDeleted) " +
        ...
        internal static readonly string DeleteForCharacter = $"DELETE FROM Quotinator_CharacterSource WHERE {IdClauses.Equals("CharacterId", "id")};";
        ...
        internal static readonly string SelectSourceReferencesForCharacter =
            $"SELECT {IdClauses.SelectColumn("s.Id", "Id")}, s.Title FROM Quotinator_CharacterSource cs " +
            $"JOIN Quotinator_Source s ON {IdClauses.Join("s.Id", "cs.SourceId")} AND s.IsDeleted = 0 " +
        ...
        internal static readonly string SelectSourceReferencesForCharacters =
            $"SELECT {IdClauses.SelectColumn("cs.CharacterId", "CharacterId")}, {IdClauses.SelectColumn("s.Id", "SourceId")}, s.Title AS SourceTitle FROM Quotinator_CharacterSource cs " +
            $"JOIN Quotinator_Source s ON {IdClauses.Join("s.Id", "cs.SourceId")} AND s.IsDeleted = 0 " +
    ...
    internal static class People
    {
        ...
        /// </summary>
        internal static readonly string SelectIdByName = $"SELECT {IdClauses.SelectColumn("Id")} FROM Quotinator_Person WHERE {TextClauses.Equals("Name", "name")} AND IsDeleted = 0;";
        ...
        /// </summary>
        internal static readonly string SelectExistingById =
            $"SELECT Name, DateOfBirth, DateOfDeath, CompletenessStatus FROM Quotinator_Person WHERE {IdClauses.Equals("Id", "id")} AND IsDeleted = 0;";
        ...
        internal static readonly string SelectCompletenessById =
            $"SELECT CompletenessStatus, NoValueKnown FROM Quotinator_Person WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_Person SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string UpdateFieldsById =
            $"UPDATE Quotinator_Person SET Name = @name, DateOfBirth = @dateOfBirth, DateOfDeath = @dateOfDeath, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string CountActiveReferences =
            $"SELECT COUNT(*) FROM Quotinator_Quote WHERE {IdClauses.Equals("PersonId", "id")} AND IsDeleted = 0;";
        ...
        /// </summary>
        internal const string InsertIfNotExists =
            "INSERT OR IGNORE INTO Quotinator_Person (Id, Name, DateOfBirth, DateOfDeath, ImportBatchId, DateCreated, DateModified, DateDeleted, IsDeleted, CompletenessStatus, NoValueKnown) " +
    ...
    internal static class Sources
    {
        ...
        internal static readonly string SelectIdByTitleAndType =
            $"SELECT {IdClauses.SelectColumn("Id")} FROM Quotinator_Source WHERE {TextClauses.Equals("Title", "title")} AND {TextClauses.Equals("Type", "type")} AND IsDeleted = 0;";
        ...
        /// <summary>Read before an apply so #165's CompletenessGuard.ComputeNextStatus can see the before-state. Case-insensitive — see <see cref="SelectExistingById"/>'s remark.</summary>
        internal static readonly string SelectCompletenessById =
            $"SELECT CompletenessStatus, NoValueKnown FROM Quotinator_Source WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string SelectSeriesIdById =
            $"SELECT {IdClauses.SelectColumn("SeriesId")} FROM Quotinator_Source WHERE {IdClauses.Equals("Id", "id")};";

        /// <summary>Persists #165's decide-time override or auto-computed transition — the only path allowed to change CompletenessStatus after insert. Case-insensitive — see <see cref="SelectExistingById"/>'s remark.</summary>
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_Source SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        /// </summary>
        internal static readonly string CountActiveReferences =
            $"SELECT (SELECT COUNT(*) FROM Quotinator_Quote WHERE {IdClauses.Equals("SourceId", "id")} AND IsDeleted = 0) " +
            $"+ (SELECT COUNT(*) FROM Quotinator_CharacterSource cs JOIN Quotinator_Character c ON {IdClauses.Join("c.Id", "cs.CharacterId")} " +
        ...
        internal static readonly string SelectSeriesReferenceForSource =
            $"SELECT {IdClauses.SelectColumn("ser.Id", "Id")}, ser.Name FROM Quotinator_Source s " +
    ...
    internal static class Series
    {
        ...
        /// </summary>
        internal static readonly string SelectIdByName = $"SELECT {IdClauses.SelectColumn("Id")} FROM Quotinator_Series WHERE {TextClauses.Equals("Name", "name")} AND IsDeleted = 0;";
        ...
        internal static readonly string SelectExistingById =
            $"SELECT Name, {IdClauses.SelectColumn("UniverseId")}, CompletenessStatus FROM Quotinator_Series WHERE {IdClauses.Equals("Id", "id")} AND IsDeleted = 0;";
        ...
        internal static readonly string UpdateFieldsById =
            $"UPDATE Quotinator_Series SET Name = @name, UniverseId = @universeId, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal const string InsertIfNotExists =
            "INSERT OR IGNORE INTO Quotinator_Series (Id, Name, UniverseId, ImportBatchId, DateCreated, DateModified, DateDeleted, IsDeleted, CompletenessStatus, NoValueKnown) " +
        ...
        internal static readonly string CountActiveReferences =
            $"SELECT COUNT(*) FROM Quotinator_Source WHERE {IdClauses.Equals("SeriesId", "id")} AND IsDeleted = 0;";

        /// <summary>Read before an apply so #165's CompletenessGuard.ComputeNextStatus can see the before-state. Case-insensitive — see <see cref="Sources.SelectExistingById"/>'s remark.</summary>
        internal static readonly string SelectCompletenessById =
            $"SELECT CompletenessStatus, NoValueKnown FROM Quotinator_Series WHERE {IdClauses.Equals("Id", "id")};";

        /// <summary>Persists #165's decide-time override or auto-computed transition — the only path allowed to change CompletenessStatus after insert. Case-insensitive — see <see cref="Sources.SelectExistingById"/>'s remark.</summary>
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_Series SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string SelectUniverseReferenceForSeries =
            $"SELECT {IdClauses.SelectColumn("u.Id", "Id")}, u.Name FROM Quotinator_Series s " +
    ...
    internal static class Universe
    {
        ...
        /// </summary>
        internal static readonly string SelectIdByName = $"SELECT {IdClauses.SelectColumn("Id")} FROM Quotinator_Universe WHERE {TextClauses.Equals("Name", "name")} AND IsDeleted = 0;";
        ...
        internal static readonly string SelectExistingById =
            $"SELECT Name, CompletenessStatus FROM Quotinator_Universe WHERE {IdClauses.Equals("Id", "id")} AND IsDeleted = 0;";
        ...
        internal static readonly string UpdateFieldsById =
            $"UPDATE Quotinator_Universe SET Name = @name, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";

        /// <summary>See <see cref="Characters.InsertIfNotExists"/>'s remark — same idempotent-Add rationale.</summary>
        internal const string InsertIfNotExists =
            "INSERT OR IGNORE INTO Quotinator_Universe (Id, Name, ImportBatchId, DateCreated, DateModified, DateDeleted, IsDeleted, CompletenessStatus, NoValueKnown) " +
        ...
        internal static readonly string CountActiveReferences =
            $"SELECT COUNT(*) FROM Quotinator_Series WHERE {IdClauses.Equals("UniverseId", "id")} AND IsDeleted = 0;";

        /// <summary>Read before an apply so #165's CompletenessGuard.ComputeNextStatus can see the before-state. Case-insensitive — see <see cref="Sources.SelectExistingById"/>'s remark.</summary>
        internal static readonly string SelectCompletenessById =
            $"SELECT CompletenessStatus, NoValueKnown FROM Quotinator_Universe WHERE {IdClauses.Equals("Id", "id")};";

        /// <summary>Persists #165's decide-time override or auto-computed transition — the only path allowed to change CompletenessStatus after insert. Case-insensitive — see <see cref="Sources.SelectExistingById"/>'s remark.</summary>
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_Universe SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
    }
    ...
    internal static class Conversations
    {
        ...
        internal static readonly string SelectForRead =
            $"SELECT {IdClauses.SelectColumn("Id")}, Description FROM Quotinator_Conversation WHERE {IdClauses.Equals("Id", "id")} AND IsDeleted = 0;";

        /// <summary>See <see cref="Characters.InsertIfNotExists"/>'s remark — same idempotent-Add rationale.</summary>
        internal const string InsertIfNotExists =
            "INSERT OR IGNORE INTO Quotinator_Conversation (Id, Description, ImportBatchId, DateCreated, DateModified, DateDeleted, IsDeleted) " +
        ...
        /// </summary>
        internal static readonly string SelectExistingById =
            $"SELECT Description, CompletenessStatus FROM Quotinator_Conversation WHERE {IdClauses.Equals("Id", "id")} AND IsDeleted = 0;";

        /// <summary>Read before an apply so #165's CompletenessGuard.ComputeNextStatus can see the before-state. Case-insensitive — see <see cref="SelectExistingById"/>'s remark.</summary>
        internal static readonly string SelectCompletenessById =
            $"SELECT CompletenessStatus, NoValueKnown FROM Quotinator_Conversation WHERE {IdClauses.Equals("Id", "id")};";

        /// <summary>Persists #165's decide-time override or auto-computed transition — the only path allowed to change CompletenessStatus after insert. Case-insensitive — see <see cref="SelectExistingById"/>'s remark.</summary>
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_Conversation SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string UpdateDescriptionById =
            $"UPDATE Quotinator_Conversation SET Description = @description, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
    }
    ...
    internal static class ConversationLines
    {
        ...
        /// </summary>
        internal static readonly string DeleteForConversation = $"DELETE FROM Quotinator_ConversationLine WHERE {IdClauses.Equals("ConversationId", "id")};";
        ...
        internal static readonly string DeleteForStageDirection = $"DELETE FROM Quotinator_ConversationLine WHERE {IdClauses.Equals("StageDirectionId", "id")};";
        ...
        internal static readonly string DeleteForSoundCue = $"DELETE FROM Quotinator_ConversationLine WHERE {IdClauses.Equals("SoundCueId", "id")};";

        internal const string Insert =
            "INSERT OR IGNORE INTO Quotinator_ConversationLine " +
        ...
        internal static readonly string SelectByConversationId =
            $"SELECT [Order], LineType, {IdClauses.SelectColumn("QuoteId")}, {IdClauses.SelectColumn("StageDirectionId")}, {IdClauses.SelectColumn("SoundCueId")} FROM Quotinator_ConversationLine " +
        ...
        internal static readonly string SelectMembershipForQuote =
            ...
            $"(SELECT COUNT(*) FROM Quotinator_ConversationLine cl2 WHERE {IdClauses.Join("cl2.ConversationId", "cl.ConversationId")} AND cl2.IsDeleted = 0) AS TotalLines " +
            "FROM Quotinator_ConversationLine cl " +
            $"INNER JOIN Quotinator_Conversation c ON {IdClauses.Join("c.Id", "cl.ConversationId")} AND c.IsDeleted = 0 " +
        ...
        internal static readonly string SelectQuoteIdsForConversation =
            $"SELECT {IdClauses.SelectColumn("QuoteId")} FROM Quotinator_ConversationLine WHERE {IdClauses.Equals("ConversationId", "conversationId")} AND QuoteId IS NOT NULL AND IsDeleted = 0;";
        ...
        internal static readonly string SelectLineCountsForConversations =
            ...
            $"CAST((SELECT COUNT(*) FROM Quotinator_ConversationLine cl2 WHERE {IdClauses.Join("cl2.ConversationId", "cl.ConversationId")} AND cl2.IsDeleted = 0) AS INTEGER) AS LineCount " +
            "FROM Quotinator_ConversationLine cl " +
    ...
    internal static class StageDirections
    {
        ...
        /// </summary>
        internal static readonly string SelectExistingById =
            $"SELECT Text, ImageUrl, CompletenessStatus FROM Quotinator_StageDirection WHERE {IdClauses.Equals("Id", "id")} AND IsDeleted = 0;";

        /// <summary>Read before an apply so #165's CompletenessGuard.ComputeNextStatus can see the before-state. Case-insensitive — see <see cref="SelectExistingById"/>'s remark.</summary>
        internal static readonly string SelectCompletenessById =
            $"SELECT CompletenessStatus, NoValueKnown FROM Quotinator_StageDirection WHERE {IdClauses.Equals("Id", "id")};";

        /// <summary>Persists #165's decide-time override or auto-computed transition — the only path allowed to change CompletenessStatus after insert. Case-insensitive — see <see cref="SelectExistingById"/>'s remark.</summary>
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_StageDirection SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string UpdateFieldsById =
            $"UPDATE Quotinator_StageDirection SET Text = @text, ImageUrl = @imageUrl, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        /// </summary>
        internal static readonly string CountActiveReferences =
            "SELECT COUNT(*) FROM Quotinator_ConversationLine cl " +
            $"INNER JOIN Quotinator_Conversation c ON {IdClauses.Join("c.Id", "cl.ConversationId")} " +
            $"WHERE {IdClauses.Equals("cl.StageDirectionId", "id")} AND c.IsDeleted = 0;";

        /// <summary>See <see cref="Characters.InsertIfNotExists"/>'s remark — same idempotent-Add rationale.</summary>
        internal const string InsertIfNotExists =
            "INSERT OR IGNORE INTO Quotinator_StageDirection (Id, Text, ImageUrl, ImportBatchId, DateCreated, DateModified, DateDeleted, IsDeleted) " +
        ...
        internal static readonly string SelectByIdWithTranslation =
            ...
            FROM Quotinator_StageDirection sd
            LEFT JOIN Quotinator_StageDirectionTranslation sdt ON {IdClauses.Join("sdt.StageDirectionId", "sd.Id")} AND {TextClauses.Equals("sdt.Language", "lang")} AND sdt.IsDeleted = 0
    ...
    internal static class StageDirectionTranslations
    {
        ...
        internal const string Insert =
            "INSERT OR IGNORE INTO Quotinator_StageDirectionTranslation " +
    ...
    internal static class SoundCues
    {
        ...
        /// </summary>
        internal static readonly string SelectExistingById =
            $"SELECT Text, SoundFileUrl, ImageUrl, CompletenessStatus FROM Quotinator_SoundCue WHERE {IdClauses.Equals("Id", "id")} AND IsDeleted = 0;";

        /// <summary>Read before an apply so #165's CompletenessGuard.ComputeNextStatus can see the before-state. Case-insensitive — see <see cref="SelectExistingById"/>'s remark.</summary>
        internal static readonly string SelectCompletenessById =
            $"SELECT CompletenessStatus, NoValueKnown FROM Quotinator_SoundCue WHERE {IdClauses.Equals("Id", "id")};";

        /// <summary>Persists #165's decide-time override or auto-computed transition — the only path allowed to change CompletenessStatus after insert. Case-insensitive — see <see cref="SelectExistingById"/>'s remark.</summary>
        internal static readonly string UpdateCompletenessById =
            $"UPDATE Quotinator_SoundCue SET CompletenessStatus = @completenessStatus, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        internal static readonly string UpdateFieldsById =
            $"UPDATE Quotinator_SoundCue SET Text = @text, SoundFileUrl = @soundFileUrl, ImageUrl = @imageUrl, DateModified = @dateModified WHERE {IdClauses.Equals("Id", "id")};";
        ...
        /// <summary>See <see cref="Characters.InsertIfNotExists"/>'s remark — same idempotent-Add rationale.</summary>
        internal const string InsertIfNotExists =
            "INSERT OR IGNORE INTO Quotinator_SoundCue (Id, Text, SoundFileUrl, ImageUrl, ImportBatchId, DateCreated, DateModified, DateDeleted, IsDeleted) " +
        ...
        internal static readonly string SelectByIdWithTranslation =
            ...
            FROM Quotinator_SoundCue sc
            LEFT JOIN Quotinator_SoundCueTranslation sct ON {IdClauses.Join("sct.SoundCueId", "sc.Id")} AND {TextClauses.Equals("sct.Language", "lang")} AND sct.IsDeleted = 0
    ...
    internal static class SoundCueTranslations
    {
        ...
        internal const string Insert =
            "INSERT OR IGNORE INTO Quotinator_SoundCueTranslation " +
