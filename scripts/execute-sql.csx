...
// database file on the host side during manual verification (e.g. docs/smoke-tests.md's #29, which
// needs to drop a table to reproduce a schema/version mismatch against a running Docker container's
// bind-mounted data directory) — never referenced by src/Quotinator.Api, never built into the Docker
// image, developer-only like DbInspector.
//
// Usage (run from repo root):
//   dotnet-script scripts/execute-sql.csx -- --db <path-to-db-file> --sql "<statement(s)>"
//
// Options:
